...
if "__name__" != "__main__":
 ...
 SVM_model = model
 SVM_train_data = SVM_model.predict(train_data[0:12000,:,:,:])
 SVM_test_data = SVM_model.predict(test_data)

 SVM_train_data = StandardScaler().fit_transform(SVM_train_data)
 SVM_test_data = StandardScaler().fit_transform(SVM_test_data)
 
 clf = SVC(C = 1,kernel='linear')
 clf.fit(SVM_train_data,train_labels_C[0:12000])
 prid  = clf.predict(SVM_test_data)
 #for i in range(len(SVM_test_data)):
 #    print int(prid[i])
 thefile = open('q2_c_output.txt', 'w')
 for i in range(len(test_data)):
     thefile.write("%s\n" % Class_names['label_names'][int(prid[i])] ) 
