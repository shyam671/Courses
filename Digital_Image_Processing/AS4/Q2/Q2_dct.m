I = (imread('Uncompressed_03.bmp'));
figure,imshow(uint8(I));
I = double(rgb2ycbcr(I));
I1 = I(:,:,1);
I2 = I(:,:,2);
I3 = I(:,:,3);
Cr = 2;
...
for J=1:size(I,1)/8
    for K=1:size(I,2)/8        
        ...
        Im_temp=round(  (dct2( double(Im_temp) - 127 ))./Q_y);
        LO = mat2lin(Im_temp);
        L = detect_last_o(LO);
        ...
        Im_temp=round(  (dct2( double(Im_temp) - 127 ))./Q_c);
        LO = mat2lin(Im_temp);
        L = detect_last_o(LO);
        Final_length = [Final_length real(L)];
        Count = Count + size(L,2);
...
I_c = cat(3,I_c1,I_c2,I_c3);
I_c = ycbcr2rgb(uint8(I_c));
figure,imshow((I_c));
%% RMS error
difference = (I) - (double(I_c));
