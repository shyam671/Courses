I = im2double(imread('Uncompressed_01.bmp'));
figure,imshow((I));
I1 = I(:,:,1);
I2 = I(:,:,2);
I3 = I(:,:,3);
...
for J=1:size(I,1)/8
    for K=1:size(I,2)/8        
        ...
        Im_temp=round(  (fwht( double(Im_temp) - 127 ,N,'hadamard'))./Q_y);
        LO = mat2lin(Im_temp);
        L = detect_last_o(LO);
        ...
        Im_temp=round(  (fwht( double(Im_temp) - 127 ,N,'hadamard'))./Q_c);
        LO = mat2lin(Im_temp);
        L = detect_last_o(LO);
        Final_length = [Final_length real(L)];
        Count = Count + size(L,2);
